...
def test_find_openalex_citing_works_funding(monkeypatch) -> None:
    """Test OpenAlex funding aggregation for citing works."""
    ...
    monkeypatch.setattr(connectedness, "get_api_data", _fake_get_api_data)
    result = find_openalex_citing_works_funding(
    ...
    assert result["citing_works_with_funding_count"] == 1
    assert result["citing_works_funding_records_count_sampled"] == 2
    assert result["citing_works_funding_amount_usd_total_sampled"] == 150
    assert result["citing_works_funding_sources_count_sampled"] == 2
    assert result["citing_works_unique_funders_count_sampled"] == 3
    assert result["citing_works_unique_funders_sampled"] == [
        "f1",
        "f2",
        "https://openalex.org/F3",
    ]
    assert result["references"][0]["title"] == "Citing Work A"
    assert result["references"][0]["funding_amount_usd_total"] == 150
